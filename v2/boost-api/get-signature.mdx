...
<Note>
  Both endpoints validate that transactions meet the boost's requirements before returning a signature. The transactions endpoint only considers transactions that occurred after the boost was created.
</Note>

## Response Structure

Both endpoints return the same response structure when a valid transaction is found:
