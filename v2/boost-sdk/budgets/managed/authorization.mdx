...
<Warning>
  Accounts authorized with the `MANAGER` roles are allowed to disburse available funds from the budget, so make sure you trust the address before granting these permissions.
</Warning>
