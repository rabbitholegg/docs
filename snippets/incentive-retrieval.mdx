...
The clawback function is used to reclaim assets from a Boost's incentive back to the associated budget. Can only be called by authorized users (admin or manager).
...
<Warning>
  The amount specified in buildClawbackData must be an exact multiple of the reward value set in the incentive. Otherwise, the transaction will fail.
</Warning>
